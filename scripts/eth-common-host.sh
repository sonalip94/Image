...
# shellcheck source=docker/scripts/eth-common.sh
source "$(dirname "${BASH_SOURCE[0]}")/eth-common.sh"
...
_run_iperf3_server() {
    ...
    local iperf_debug_flags=()
    if [ ${debug_mode} ]; then
        iperf_debug_flags=("--verbose" "--timestamps")
    fi

    echo -n "Turning on iperf3 server for both network namespaces..."
    # shellcheck disable=SC2068
    ip netns exec nw_ns0 iperf3 -s -D -1 -B "${ip_eth0}" -p 5678 ${iperf_debug_flags[@]} &
    # shellcheck disable=SC2068
    ip netns exec nw_ns1 iperf3 -s -D -1 -B "${ip_eth1}" -p 5678 ${iperf_debug_flags[@]} &

    while [ "$(ip netns exec nw_ns0 netstat -tulpn | grep iperf3)" == "" ] || \
           [ "$(ip netns exec nw_ns1 netstat -tulpn | grep iperf3)" == "" ] ; do
...
_run_performance_test() {
    ...
    if [ "${duplex}" = "full" ]; then
        ...
        "${duration}" -B "${ip_eth1}" --cport 5001 -c "${ip_eth0}" -p 5678 ${iperf_debug_flags[@]} > "${nw1_log}" &
        # shellcheck disable=SC2068
        ip netns exec nw_ns0 iperf3 -A 2,3 -4 "${iperf_stream}" -b 0 -l "${payload_size}" -t \
        "${duration}" -B "${ip_eth0}" --cport 5001 -c "${ip_eth1}" -p 5678 ${iperf_debug_flags[@]} > "${nw0_log}" &
        wait
    else
        # shellcheck disable=SC2068
        ip netns exec nw_ns1 iperf3 -A 0,1 -4 "${iperf_stream}" -b 0 -l "${payload_size}" -t \
...
# log_file is an optional parameter.
# shellcheck disable=SC2120
_get_target_load_log() {
    local log_file="${1:-${sar_log}}"
    ...
    echo -e "Copy ${log_file} from target to host via nw_ns0:"
    ip netns exec nw_ns0 scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    root@10.0.1.100:"${log_file}" "${log_file}"
}

# Copy log from the target to the host using serial
_copy_file_over_serial() {
    local file_to_copy="$1"
    local destination_file="$2"
    local minicom_script="/tmp/minicom.script"
    name_file_to_copy="$(basename "${file_to_copy}")"

    #Create minicom script file to copy log from board to host machine
    echo "send \"sz ${file_to_copy} > /dev/ttyLF0\""  > "${minicom_script}"
    echo "sleep 1" >> "${minicom_script}"
    # Start copy process
    minicom -D "${uart_dev}" -S "${minicom_script}" <<< ^C > /dev/null 2>&1
    sync
    # Clear temp files in host
    rm "${minicom_script}"
    mv "${name_file_to_copy}" "${destination_file}"
}
