...
_run_tcpreplay() {
    ...
    tcpreplay -l "$((times_to_run+1))" -p "${speed}" -i "${eth_idps}" "${pcap_file}" > "${idps_host_log}"

    # Wait for execution to finish.
    sleep "${tcpreplay_timeout}"
    # Flush all data in the buffer of the target log file
    echo "sync" > "${uart_dev}"
    ...
    # Wait 3 seconds for killing sar process
    sleep 3
}
