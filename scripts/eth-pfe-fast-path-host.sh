...
_disable_hw_bridge_target() {
    if [ "${layer_number}" -eq 2 ]; then
        echo "libfci_cli bd-remif --vlan" ${vlan_id} "--i emac0"> "${uart_dev}"
        echo "libfci_cli bd-remif --vlan" ${vlan_id} "--i emac2"> "${uart_dev}"
        echo "libfci_cli bd-update --vlan" ${vlan_id} "--ucast-hit 3 --ucast-miss 3 --mcast-hit 3 --mcast-miss 3" > "${uart_dev}"
    fi
}
